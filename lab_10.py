...
Original file is located at
    https://colab.research.google.com/drive/1kvnlKrSN9oUR4gmMaUZH_ib4WsuZww3i
"""
...
def make_pipeline():
    ...
    fq0_eps_act = fq0_eps.actual_value.latest
    ...
    estimated_growth_winsorized=estimated_growth_factor.winsorize(min_percentile=0.05, max_percentile=0.95)
    value_winsorized = value.winsorize(min_percentile=0.01, max_percentile=0.99)
    quality_winsorized = quality.winsorize(min_percentile=0.01, max_percentile=0.99)
    sentiment_score_winsorized = sentiment_score.winsorize(
        min_percentile=0.01,
        max_percentile=0.99
        )
    last_close_price_winsorized= last_close_price.winsorize(
        min_percentile=0.01,
        max_percentile=0.99
        )
    fq0_eps_act_winzorized = fq0_eps_act.winsorize(
        min_percentile=0.01,
        max_percentile=0.99
        )
    ...
    if(close_price_filter ==1):
        combined_factor = (
        value_winsorized.zscore() +
        quality_winsorized.zscore() +
        sentiment_score_winsorized.zscore()+
        estimated_growth_winsorized.zscore()+
        last_close_price_winsorized.zscore()+
        fq0_eps_act_winzorized.zscore()
